...
export default function AboutSection1() {
  ...
  return (
    ...
    >
      ...
      {/* Atmospheric Background & Low Opacity Grid */}
      <TimelineContent
        className="absolute inset-0 z-0 bg-[linear-gradient(to_right,#ffffff02_1px,transparent_1px),linear-gradient(to_bottom,#ffffff02_1px,transparent_1px)] bg-[size:70px_70px] [mask-image:radial-gradient(ellipse_80%_50%_at_50%_100%,#000_70%,transparent_110%)]"
        animationNum={3}
      ...
      <div className="relative z-10 max-w-3xl mx-auto">
        <div className="text-gold text-xs font-bold uppercase tracking-[0.4em] mb-6 flex items-center justify-center gap-2">
          <div className="h-[1px] w-6 bg-gold/30" />
          Our Heritage
          <div className="h-[1px] w-6 bg-gold/30" />
        </div>

        <h2 className="text-4xl sm:text-5xl md:text-6xl font-marcellus text-beige mb-6 text-center leading-tight">
          <VerticalCutReveal
            splitBy="words"
            staggerDuration={0.2}
            staggerFrom="last"
          ...
          >
            {"A Legacy of Culinary Excellence, How Our Passion Fuels Every Dish"}
          </VerticalCutReveal>
        </h2>
        ...
        <TimelineContent
          ...
          timelineRef={heroRef}
          className="text-beige-muted/70 text-center text-sm sm:text-lg mb-8 leading-relaxed max-w-2xl mx-auto"
        >
          For over 15 years, our mission has been to bring the authentic flavors of Yemeni slow-cooking and legendary Arabian hospitality to food lovers. We are committed to hand-picked spices, ancient underground ovens, and best-in-class service.
        </TimelineContent>
        ...
        >
          <a
            href="/?menu=open"
            className="bg-maroon hover:bg-maroon-light shadow-2xl border border-gold/20 flex w-fit gap-2 hover:gap-4 transition-all duration-300 ease-in-out text-white px-8 py-4 rounded-full cursor-pointer text-xs font-bold tracking-[0.2em] uppercase"
          >
